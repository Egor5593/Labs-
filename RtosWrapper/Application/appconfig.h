...
const auto sampleTime = 100ms;
const auto rc = 1000ms;
...
RCFilter rcFilter(static_cast<float>(sampleTime.count()), static_cast<float>(rc.count()));

LedManagerTask ledManagerTask (modeManager, 500ms, mailBox);
ButtonTask buttonTask(button, 200ms, mailBox);
MeasureTask measureTask(sampleTime, rcFilter);
